...
"""Different Dicce types added to integrate polymorphism"""
...
class Game:
    ...
    def play(self):
        ...
        # Announce winners
        if len(winners) > 1:
            print(f"{Fore.CYAN}It's a tie! The winners are: {', '.join(
                winners)} with {max_score} points each!{Style.RESET_ALL}\n")
        else:
            print(f"{Fore.CYAN}The winner is {winners[0]} with {
                  max_score} points!{Style.RESET_ALL}\n")
